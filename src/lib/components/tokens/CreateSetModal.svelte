<script>
  import { createTokenSet, notify } from '$lib/stores/platform.svelte.js';
  ...
  async function handleCreate() {
    ...
    saving = true;
    try {
      await createTokenSet(name.trim(), projectId);
      onclose();
    } catch {
    ...
    <div class="modal-body">
      <div class="form-group">
        ...
        <input
          ...
          type="text"
          bind:value={name}
          placeholder="Core/Colors, Brand/Light..."
          class="form-input mono"
          autofocus
          onkeydown={e => e.key === 'Enter' && handleCreate()}
        />
        <span class="form-hint">Use slashes for folder groups (e.g., Core/Colors)</span>
      </div>
    </div>
